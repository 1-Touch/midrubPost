    ...
    public function authenticate_user($args) {
        ...
        if ( preg_match('/@/i', $args['email']) ) {
            ...
            if ($user) {
                ...
                // Get member
                $team_owner = $this->CI->base_model->get_data_where('teams', '*', array('member_email' => strtolower($args['email'])));
                ...
                if ( $team_owner ) {
                    ...
                    if ( $user_data ) {
                        ...
                        // Check if user and password exists
                        if ($this->CI->base_users->check_user($user_data[0]['username'], $args['password'])) {
                            ...
                            if ( $user_data[0]['status'] == 2 ) {
                                ...
                                // Create the session
                                $this->CI->session->set_userdata('username', $user_data[0]['username']);
            ...
            if ($team_owner) {
                ...
                if ( $user_data[0]->status === 2 ) {
                    ...
                    // Create the session
                    $this->CI->session->set_userdata('username', $team_owner['username']);

                    // Create the team's member session
                    $this->CI->session->set_userdata('member', $args['email']);
